...
// จัดการการลบข้อมูล (Delete) พร้อมลบไฟล์รูปภาพออกจากโฟลเดอร์
if (isset($_GET['delete_id'])) {
    // ดึงชื่อไฟล์ภาพเก่ามาลบก่อน
    $stmtImg = $pdo->prepare("SELECT image FROM foods WHERE id = ?");
    $stmtImg->execute([$_GET['delete_id']]);
    $imgData = $stmtImg->fetch();
    if ($imgData && $imgData['image'] && file_exists("uploads/" . $imgData['image'])) {
                                ...
                                <tr>
                                        ...
                                        <?php if (!empty($recipes)): ?>
                                            <ul class="mb-0 ps-3">
                                                <?php foreach ($recipes as $r): ?>
                                                    <li><?= htmlspecialchars($r['recipe_name']) ?>                 <?= $r['quantity'] ?>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="index.php?delete_id=<?= $food['id'] ?>" class="btn btn-sm btn-danger"
                                            onclick="return confirm('คุณแน่ใจหรือไม่ที่จะลบเมนูนี้? ข้อมูลและรูปภาพจะถูกลบไปด้วย');">ลบ</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
